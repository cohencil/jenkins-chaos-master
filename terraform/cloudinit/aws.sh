...
su - ${user} -c "mkdir ~/.ssh/github"
su - ${user} -c "aws ssm get-parameter --name /JENKINS_CHAOS_MASTER/GITHUB_CLIENT_SERCRET --with-decryption --query 'Parameter.Value' --output text > ~/.ssh/github/secret"
